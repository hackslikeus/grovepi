...
# Call MPD Display if Running
    if mpc_status() == 0:
        ...
        # Display Data
        lcd.setCursor(0,0);
        lcd.write(datetime.now().strftime('%m/%d/%Y %I:%M'))
        lcd.setCursor(1,0);
        lcd.write(ip_address())
        lcd.write("CPU: "+measure_temp()+"c "+cpu_load()+"%")
#       lcd.write(" ",LCD_LINE_3,1)
        lcd.write("RAM FREE: "+ram_used()+"Mgb")
        time.sleep(1)
    elif mpc_status() == 1:
        if (len(current_artist()) == 0):
            ...
            artist = astring.split('-')[0]
            lcd.setCursor(0,0);
            lcd.write(artist.strip())
            lcd.setCursor(1,0);
            lcd.write(song.strip())
            #lcd.write(song_elapsed()+" / Stream Time")
        else:
            lcd.write(current_artist())
            lcd.setCursor(0,0);
            lcd.write(current_album())
            lcd.setCursor(0,1);
            lcd.write(current_song())
            lcd.write(song_elapsed()+"/"+current_time())
